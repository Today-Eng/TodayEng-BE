...
public class AnswerPersistenceService {
    ...
    public void validateAnswerable(Long userId, Long diaryId, Long questionId) {
        ...
        if (answerRepository.existsByQuestionId(questionId)) throw new BaseException(ErrorCode.ANSWER_ALREADY_EXISTS);
    }

    @Transactional
    public DiaryAnswer createUploaded(Long userId, Long diaryId, Long questionId, String key) {
        validateAnswerable(userId, diaryId, questionId);
        try {
            return answerRepository.saveAndFlush(DiaryAnswer.createUploaded(
