...
- 모든 API 응답은 `com.example.todayEng.global.common.ApiResponse<T>`로 감싸서 반환해야 합니다.
...
- 새로운 에러 상황이 필요하면 `com.example.todayEng.global.error.ErrorCode` enum에 규칙에 맞는 에러 코드를 먼저 추가하십시오.
